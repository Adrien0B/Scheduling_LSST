...
def factibles( X,observer,actual ):
	...
	I=np.ones(np.size(X,0),dtype=np.int) #Idicates if factible or not.	
	...
	#Save indices of the ones in I
	Factible=np.zeros(I.sum(),int)
	k=0	
	for j in range(0,np.size(I,0)):
		if I[j]==1:
			Factible[k]=j
			k=k+1
	return Factible
def Time_dist(X,observer,actual):
	az_vel=10
	alt_vel=10
	X_obs=np.transpose(np.array(conversion(obs.lat,X[:,0],X[:,1],float(observer.sidereal_time())))) #[AZ,ALT] of X for the observer.
	actual_AA=np.transpose(np.array(conversion(obs.lat,actual[0],actual[1],float(observer.sidereal_time())))) #[AZ,ALT] of actual point.
...
#Using list T, find Ranking 2. 
    ...
    return [zen1, rk2]
        ...
        #a futuro evaluar si es necesario un m�todo de resoluci�n de empates. 
...
#X=np.array([[0.453,0.567],[0.234,0.1],[-0.23,0.67],[-0.3,-0.3]])
#print X
obs=ephem.Observer()
obs.lat="-33:27:00"
obs.lon="-70:40:00"
...
#Generar 600 puntos con DEC en [-pi/2,0] RA en [0,2*pi], generar T aleatorio. Probar Factible_Schedule.
Y=np.array([[1,-0.5],[1,0.5],[1,0.5]])
X=np.transpose(conversion(obs.lat,Y[:,0],Y[:,1],obs.sidereal_time()))
print Y
print X
Z=np.array(obs.radec_of(X[0,0],X[0,1]))
print Z




#X=np.transpose(np.array(hpy.pixelfunc.pix2ang(32,np.arange(6080,12288,1))))
#X[:,0]=np.pi/2-X[:,0]
#X[:,1]-=np.pi
#Num=np.size(X,0)

X=np.transpose(np.array(hpy.pixelfunc.pix2ang(16,np.arange(1504,3071,1))))
X[:,0]=np.pi/2-X[:,0]
X[:,1]-=np.pi
Num=np.size(X,0)

#T = np.random.randint(Num, size=Num)
T=4*np.ones(Num)

[sched,schedAA] = Factible_Schedule(X,obs,T)
...
if __name__ == '__main__':
    ...
    plt.subplot(211, projection="custom_hammer")
#    p = plt.plot(sched[0:200,1], sched[0:200,0],'o-')#(1,sched.size/2,sched.size/2),s=50)
    p = plt.scatter(sched[0:SS-1,1], sched[0:SS-1,0],c=np.linspace(1,SS-1,SS-1),s=50, alpha=50, edgecolor='none',cmap='viridis')#(1,sched.size/2,sched.size/2),s=50)
    plt.grid(True)

    plt.subplot(212, projection="polar")
    p2 = plt.scatter(schedAA[0:SS-1,0],np.pi/2-schedAA[0:SS-1,1],c=np.linspace(1,SS-1,SS-1),s=50, alpha=0.5, edgecolor='none',cmap='viridis')#(1,sched.size/2,sched.size/2),s=50)
    #p2 = plt.scatter(schedAA[:,0],np.pi/2-schedAA[:,1],s=50)#(1,sched.size/2,sched.size/2),s=50)
